layui.define(['table', 'jquery', 'element', 'form', 'tab', 'menu', 'frame'],
	function(exports) {
		...
		$("body").on("click", '[role-menu-id]', function() {
			if (config.tab.muiltTab) {
				bodyTab.addTabOnly({
					id: $(this).attr("role-menu-id"),
					title: $(this).attr("role-menu-title"),
					url: $(this).attr("role-menu-url"),
					icon: "",
					close: true
				}, 300);
			} else {
				bodyFrame.changePage($(this).attr("role-menu-url"), "", true);
			}
		});
